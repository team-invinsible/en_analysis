...
# Méthode pour savoir si un float == NaN (sera utilisé si pas de F0 détectée sur une syllabe donnée)
def isNaN(num):
    return num!=num
...
#
spk2nbWsyll = {} # Number of target words per speaker (all plurisyllabic words with matching count of syllable)
spk2nbWpluri = {} # Number of plurisyllabic words per speaker
stressTable = []

# For statistics only:
spk2nbPlainPluri = {} # number of plurisyllabic plain words per speaker
spk2nbPlainTargetWord = {} # number of plurisyllabic plain target words per speaker
...
for file in os.listdir(input_folder):
    ...
    # LOOP ON INTERVALLES → CHECK IF PLURISYLLABIC WORD & WORD IN DICTIONARY → CHECK NB OF SYLLABLE NUCLEI ; IF MATCH: ACOUSTIC MEASURES ON EACH SYLLABLE
    for i,intervalle in enumerate(grid['WOR']):
        ...
        if lab != "<p:>" and lab != "":
            ...
            # Si le mot est dans le dico des mots plurisyllabiques
            if lab.lower() in gabarits.keys():
                ...
                nbWplurisyll+=1
                nbSyll=0
                syllxpos = [] # liste du xpos de chaque noyau syllabique
                sylldB = [] # liste de intensité de chaque noyau syllabique
                syllF0 = [] # liste du F0 de chaque noyau syllabique
                # on compte le nb de syllabes détectées
                for syll in grid['Nuclei']:
                    if syll.xpos >= deb and syll.xpos <= fin:
                        nbSyll+=1
                        syllxpos.append(syll.xpos)
                ...
                if ok:
                    ...
                    # GetPOS
                    posIntNb = call(tg, "Get interval at time...", 1, syllxpos[0])
                    ...
                    for s in syllxpos:
                        dB = call(intensity, "Get value at time...", s, "Nearest")
                        F0 = call(pitch, "Get value at time...", s, "Hertz", "Nearest")
                        if isNaN(F0):
                            F0 = call(PitchTier, "Get value at time...", s)
                        sylldB.append(dB)
                        syllF0.append(F0)

                    ### Duration
                    ### Calculer distance entre les syllabes :   xmin......xpos1..|..xpos2...|...xpos3..xmax
                    sylldur = []
                    for i,s in enumerate(syllxpos):
                        if i==0: 
                            sylldur.append( (s+(syllxpos[i+1]-s)/2) - deb ) # de xmin à s+moitié(s,s+1)
                        elif i==len(syllxpos)-1: 
                            sylldur.append( fin - (s-(s-syllxpos[i-1])/2) ) # de s-moitié(s-1,s) à xmax
                        else: 
                            sylldur.append( (s+(syllxpos[i+1]-s)/2) - (s-(s-syllxpos[i-1])/2) ) # de s-moitié(s-1,s) à s+moitié(s,s+1)
