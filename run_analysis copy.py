...
async def run_analysis(s3_key: str):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            logger.info(f"Analysis method completed for user {user_id}, question {question_num}")
            
            # 3. 데이터베이스 저장 완료 확인
            logger.info("Verifying database storage completion...")
            
            # 저장된 데이터 확인을 위해 잠시 대기
            await asyncio.sleep(1)
            
            # 실제로 데이터가 저장되었는지 확인
            saved_result = await analyzer.get_analysis_result(user_id, question_num)
            if saved_result:
                logger.info(f"✅ DATABASE STORAGE CONFIRMED: Analysis data successfully saved for user {user_id}, question {question_num}")
                logger.info(f"✅ ANALYSIS FULLY COMPLETED: All processing and storage finished for S3 key: {s3_key}")
                
                return {
                    "analysis_id": f"{user_id}_{question_num}",
                    "status": "completed",
                    "message": "영어 분석 및 데이터베이스 저장이 성공적으로 완료되었습니다.",
                    "user_id": user_id,
                    "question_num": question_num
                }
            else:
                logger.error(f"❌ DATABASE STORAGE FAILED: No data found after analysis for user {user_id}, question {question_num}")
                raise HTTPException(status_code=500, detail="분석은 완료되었으나 데이터베이스 저장 확인 실패")
