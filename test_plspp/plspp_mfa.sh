...
# Praat 실행 파일 경로 설정
PRAAT_PATH="/Applications/Praat.app"
...
# Praat를 위한 절대 경로 변환 및 개별 파일 처리
cd "$SCRIPT_DIR"
for audio_file in audio/*.wav; do
    if [ -f "$audio_file" ]; then
        echo "Processing: $audio_file"
        "$PRAAT_PATH/Contents/MacOS/Praat" --run "$SCRIPT_DIR/scripts/SyllableNucleiv3_DeJongAll2021.praat" "$audio_file" "Band pass (300..3300 Hz)" -25 2 0.3 "no" "English" 1.00 "TextGrid(s) only" "OverWriteData" "yes"
    fi
done

# TextGrid 파일들을 syll 폴더로 이동
mv audio/*.TextGrid syll/ 2>/dev/null || true
...
# MFA 실행 및 오류 처리
if mfa align "$SCRIPT_DIR/audio/" english_us_arpa english_us_arpa "$SCRIPT_DIR/tgmfa/" --clean --beam 100; then
    echo "✅ MFA alignment completed successfully"
else
    echo "⚠️ MFA alignment failed, trying with increased beam width..."
    if mfa align "$SCRIPT_DIR/audio/" english_us_arpa english_us_arpa "$SCRIPT_DIR/tgmfa/" --clean --beam 200; then
        echo "✅ MFA alignment completed with increased beam width"
    else
        echo "❌ MFA alignment failed. Continuing with available data..."
    fi
fi
...
# 생성된 파일들 디버깅 정보
echo "📁 생성된 파일 목록:"
ls -la "$SCRIPT_DIR"/*.csv 2>/dev/null || echo "   ⚠️ CSV 파일이 없습니다"

echo "📁 중간 폴더 확인:"
ls -la "$SCRIPT_DIR"/ | grep "^d" | grep -E "(whisperx|syll|tg|shape|benepar)"

# stressTable.csv와 pauseTable.csv 내용 확인
if [ -f "$SCRIPT_DIR/stressTable.csv" ]; then
    echo "✅ stressTable.csv 발견 ($(wc -l < "$SCRIPT_DIR/stressTable.csv") 줄)"
    echo "   헤더: $(head -1 "$SCRIPT_DIR/stressTable.csv")"
else
    echo "❌ stressTable.csv 없음"
fi

if [ -f "$SCRIPT_DIR/pauseTable.csv" ]; then
    echo "✅ pauseTable.csv 발견 ($(wc -l < "$SCRIPT_DIR/pauseTable.csv") 줄)"
    echo "   헤더: $(head -1 "$SCRIPT_DIR/pauseTable.csv")"
else
    echo "❌ pauseTable.csv 없음"
fi
...
if [ -f "$SCRIPT_DIR/stressTable.csv" ] && [ -f "$SCRIPT_DIR/pauseTable.csv" ]; then
    ...
    echo "⚠️  필수 파일이 생성되지 않았습니다. 디버깅을 위해 중간 파일을 보존합니다"
    echo "🔍 디버깅 정보:"
    echo "   • shape/ 폴더: $(ls -la "$SCRIPT_DIR/shape/" 2>/dev/null | wc -l) 파일"
    echo "   • text/ 폴더: $(ls -la "$SCRIPT_DIR/text/" 2>/dev/null | wc -l) 파일"
    echo "   • whisperx/ 폴더: $(ls -la "$SCRIPT_DIR/whisperx/" 2>/dev/null | wc -l) 파일"
fi
