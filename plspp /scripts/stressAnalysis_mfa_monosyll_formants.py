###############
#
# stressAnalysis_mfa_monosyll_formants.py
#
# This new version of stressAnalysis.py is based on stressAnalysis_mfa_monosyllabic version, but also exports formant information (mean, min, max, sd) for F1, F2, F3 of each vowel interval.
#
                ...
                if ok:
                    ...
                    for s in vowelIntervals:
                        ...
                        if len(F0list)>1:
                            ...
                            F0sd = F0min
                        syllF0min.append(F0min)
                        syllF0max.append(F0max)
                        syllF0sd.append(F0sd)
                        ...
                        ### Formants F1, F2, F3
                        F1list = [call(formant, "Get value at time...", 1, x, "Hertz", "Linear") for x in numpy.arange(s.xmin, s.xmax, time_step)]
                        F2list = [call(formant, "Get value at time...", 2, x, "Hertz", "Linear") for x in numpy.arange(s.xmin, s.xmax, time_step)]
                        F3list = [call(formant, "Get value at time...", 3, x, "Hertz", "Linear") for x in numpy.arange(s.xmin, s.xmax, time_step)]
                    
                        F1mean = statistics.mean(F1list)
                        F1min, F1max = min(F1list), max(F1list)
                        if len(F1list)>1:
                            F1sd = statistics.stdev(F1list)
                        else:
                            F1sd = F1min
                        syllF1mean.append(F1mean)
                        syllF1min.append(F1min)
                        syllF1max.append(F1max)
                        syllF1sd.append(F1sd)

                        F2mean = statistics.mean(F2list)
                        F2min, F2max = min(F2list), max(F2list)
                        if len(F2list)>1:
                            F2sd = statistics.stdev(F2list)
                        else:
                            F2sd = F2min
                        syllF2mean.append(F2mean)
                        syllF2min.append(F2min)
                        syllF2max.append(F2max)
                        syllF2sd.append(F2sd)

                        F3mean = statistics.mean(F3list)
                        F3min, F3max = min(F3list), max(F3list)
                        if len(F3list)>1:
                            F3sd = statistics.stdev(F3list)
                        else:
                            F3sd = F3min
                        syllF3mean.append(F3mean)
                        syllF3min.append(F3min)
                        syllF3max.append(F3max)
                        syllF3sd.append(F3sd)
    ...
    for stress in stressTable:
        ...
        st.write("{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{};{}\n"\
                        .format(stress['spk'],
                                ...
                                stressSyllableDeciledur,
                                meanUnstressSyllablesDecilesdur,
                                ", ".join([str(round(x,3) if isinstance(x, float) else x) for x in stress['F0min']]),
                                ", ".join([str(round(x,3) if isinstance(x, float) else x) for x in stress['F0max']]),
                                ", ".join([str(round(x,3) if isinstance(x, float) else x) for x in stress['F0sd']]),
                                ", ".join([str(round(x,3) if isinstance(x, float) else x) for x in stress["F1mean"]]),
                                ", ".join([str(round(x,3) if isinstance(x, float) else x) for x in stress["F1min"]]),
                                ", ".join([str(round(x,3) if isinstance(x, float) else x) for x in stress["F1max"]]),
                                ", ".join([str(round(x,3) if isinstance(x, float) else x) for x in stress["F1sd"]]),
                                ", ".join([str(round(x,3) if isinstance(x, float) else x) for x in stress["F2mean"]]),
                                ", ".join([str(round(x,3) if isinstance(x, float) else x) for x in stress["F2min"]]),
                                ", ".join([str(round(x,3) if isinstance(x, float) else x) for x in stress["F2max"]]),
                                ", ".join([str(round(x,3) if isinstance(x, float) else x) for x in stress["F2sd"]]),
                                ", ".join([str(round(x,3) if isinstance(x, float) else x) for x in stress["F3mean"]]),
                                ", ".join([str(round(x,3) if isinstance(x, float) else x) for x in stress["F3min"]]),
                                ", ".join([str(round(x,3) if isinstance(x, float) else x) for x in stress["F3max"]]),
                                ", ".join([str(round(x,3) if isinstance(x, float) else x) for x in stress["F3sd"]]),
